...
void main()
{
	...
	float tmp =  visibility * inShadow * diffuse *  0.8 * ambientOcclusion ;//* (diffuse + specular);
	...
	//if (defaultColor.x < 0.1 && defaultColor.y < 0.1 && defaultColor.z < 0.1) discard;
	
	color = tmp * defaultColor;
	color.w = 1.0;
	//color.rgb = defaultColor.xyz;
